...
def office_augment_pool():
    ...
    augs = [
        ...
        (ShearX, 0.3, None),
        (ShearY, 0.3, None),
        (TranslateXConst, 224 // 8, None),
        (TranslateYConst, 224 // 8, None),
        (CutoutConst, 224 // 8, None),
    ]
    return augs

def usps_augment_pool():
    # Test
    augs = [
        (AutoContrast, None, None),
        (Equalize, None, None),
...
class RandAugmentUSPS(object):
    def __init__(self, n, m, resample_mode=PIL.Image.BICUBIC, fill=(128,)):
        global RESAMPLE_MODE, FILL_COLOR
        RESAMPLE_MODE = resample_mode
        FILL_COLOR = fill
        self.n = int(n)
        self.m = m
        self.augment_pool = usps_augment_pool()
    ...
    def __call__(self, img):
        ...
        for op, max_v, bias in ops:
            ...
            if random.random() <= prob:
                img = op(img, v=self.m, max_v=max_v, bias=bias)
        img = CutoutConst(img, v=self.m, max_v=28 // 3)
        return img
class RandAugmentOFFICE(object):
    def __init__(self, n, m, resample_mode=PIL.Image.BICUBIC, fill=(128,128,128)):
        global RESAMPLE_MODE, FILL_COLOR
        RESAMPLE_MODE = resample_mode
        FILL_COLOR = fill
        self.n = int(n)
        self.m = m
        self.augment_pool = office_augment_pool()
    ...
    def __call__(self, img):
        ...
        for op, max_v, bias in ops:
            ...
            if random.random() <= prob:
                img = op(img, v=self.m, max_v=max_v, bias=bias)
        img = CutoutConst(img, v=self.m, max_v=224 // 3)
        return img
class RandAugmentSVHN(object):
    def __init__(self, n, m, resample_mode=PIL.Image.BICUBIC, fill=(128,128,128)):
        global RESAMPLE_MODE, FILL_COLOR
        RESAMPLE_MODE = resample_mode
        FILL_COLOR = fill
    ...
    def __call__(self, img):
        ...
        for op, max_v, bias in ops:
            ...
            if random.random() <= prob:
                img = op(img, v=self.m, max_v=max_v, bias=bias)
        img = CutoutConst(img, v=self.m, max_v=32 // 3)
        return img
