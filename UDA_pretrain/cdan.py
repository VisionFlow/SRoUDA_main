...
# https://github.com/thuml/Transfer-Learning-Library/tree/master/examples/domain_adaptation/image_classification/cdan.py
...
sys.path.append('.')
import utils
from ..utils import get_Mask_data
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, model: ImageClassifier,
          domain_adv: ConditionalDomainAdversarialLoss, optimizer: SGD, lr_scheduler: LambdaLR, epoch: int, args: argparse.Namespace):
    ...
    for i in range(args.iters_per_epoch):
        ...
        y, f = model(x)
        y_s, y_t, y_t_RMA = y.chunk(3, dim=0)
        f_s, f_t, f_t_RMA = f.chunk(3, dim=0)

        cls_loss = F.cross_entropy(y_s, labels_s)
        transfer_RMA_loss = domain_adv(y_s, f_s, y_t_RMA, f_t_RMA)
